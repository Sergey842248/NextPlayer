        ...
        object : ScaleGestureDetector.SimpleOnScaleGestureListener() {
            ...
            override fun onScaleBegin(detector: ScaleGestureDetector): Boolean {
                ...
                if (currentGestureAction == null) {
                    ...
                    // Initialize pan position when starting zoom gesture
                    panStartX = detector.focusX
                    panStartY = detector.focusY
                    currentPanX = exoContentFrameLayout.translationX
                    currentPanY = exoContentFrameLayout.translationY
                }
                return true
            }
    ...
    init {
        playerView.setOnTouchListener { _, motionEvent ->
            ...
            when (motionEvent.actionMasked) {
                MotionEvent.ACTION_DOWN -> {
                    if (motionEvent.pointerCount == 2) {
                        // Initialize pan tracking for 2-finger gestures
                        panPointerId1 = motionEvent.getPointerId(0)
                        panPointerId2 = motionEvent.getPointerId(1)
                        isPanning = true
                        panStartX = (motionEvent.getX(0) + motionEvent.getX(1)) / 2
                        panStartY = (motionEvent.getY(0) + motionEvent.getY(1)) / 2
                        currentPanX = exoContentFrameLayout.translationX
                        currentPanY = exoContentFrameLayout.translationY
                    }
                }
                MotionEvent.ACTION_MOVE -> {
                    if (motionEvent.pointerCount == 2 && isPanning && isZoomed) {
                        // Handle pan gesture for zoomed video
                        val currentX = (motionEvent.getX(0) + motionEvent.getX(1)) / 2
                        val currentY = (motionEvent.getY(0) + motionEvent.getY(1)) / 2

                        val deltaX = currentX - panStartX
                        val deltaY = currentY - panStartY

                        // Calculate new pan position
                        val newPanX = currentPanX + deltaX
                        val newPanY = currentPanY + deltaY

                        // Apply pan constraints based on zoom level and video dimensions
                        val maxPanX = (exoContentFrameLayout.width * exoContentFrameLayout.scaleX - exoContentFrameLayout.width) / 2
                        val maxPanY = (exoContentFrameLayout.height * exoContentFrameLayout.scaleY - exoContentFrameLayout.height) / 2

                        exoContentFrameLayout.translationX = newPanX.coerceIn(-maxPanX, maxPanX)
                        exoContentFrameLayout.translationY = newPanY.coerceIn(-maxPanY, maxPanY)
                    }
                }
            ...
            when (motionEvent.pointerCount) {
                ...
                2 -> {
                    // Handle both zoom and pan for 2-finger gestures
                    zoomGestureDetector.onTouchEvent(motionEvent)

                    // Also handle pan if we're in zoom mode
                    if (isZoomed && motionEvent.actionMasked == MotionEvent.ACTION_MOVE) {
                        val currentX = (motionEvent.getX(0) + motionEvent.getX(1)) / 2
                        val currentY = (motionEvent.getY(0) + motionEvent.getY(1)) / 2
